...
class NATransformerDecoder(FairseqNATDecoder):
    ...
    def forward(self, normalize, encoder_out, prev_output_tokens, step=0, **unused):
        ...
        ## show attention
        # id=-1
        # for i,val in enumerate(prev_output_tokens.tolist()[0]):
        #      if val==3:
        #         id=i
        #         break  
        # attn_weight=_['attn'].tolist()[0][id]
        # attentionFile='/home/v-yukliang/mycontainer/yukangliang/Projects/natasr/outputs/attention/cmlm'
        # with open(attentionFile,'a+') as f:
        #     f.write("%d\t%s\n" % (id,json.dumps(attn_weight)))
        ...
        **unused
        ...
        for i, layer in enumerate(self.layers):
            ...
            x, attn, _ = layer(
                ...
                need_attn=bool((i == alignment_layer)),
                need_head_weights=bool((i == alignment_layer)),

            )
            inner_states.append(x)

            if attn is not None and i == alignment_layer:
                    attn = attn.float().to(x)
