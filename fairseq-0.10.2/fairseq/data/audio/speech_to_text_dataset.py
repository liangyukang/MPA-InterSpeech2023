...
class SpeechToTextDataset(FairseqDataset):
    ...
    def __getitem__(
        ...
        score = None
        if self.scores[index]!='':
            score = json.loads(self.scores[index])
            
            ## phoneme
            # score.append(0.0)
            # score = torch.FloatTensor(score)
            # assert score.size()==target.size()
            
            ## word
            # new_score=[]

            # subwords = tokenized.split(' ')
            # special=subwords[0][0]

            # id = 0
            # for subword in subwords:
            #     if subword.startswith(special):
            #         new_score.append(score[id])
            #     else:
            #         id-=1
            #         new_score.append((score[id]))
            #     id+=1
            # assert len(new_score) == len(subwords)
            # new_score.append(0.0)
            # score = torch.FloatTensor(new_score)

        label = None
        if self.labels[index]!='':
            label = json.loads(self.labels[index])
        
        
        return index, source, target, label
    ...
    def collater(self, samples) -> Dict:
        ...
        if self.tgt_texts is not None:
            ...
            # score = fairseq_data_utils.collate_tokens(
            ...
            # label = torch.tensor([s for _, _, _,s in samples], dtype=torch.long)
...
class SpeechToTextDatasetCreator(object):
    ...
    # default values
    DEFAULT_SPEAKER = DEFAULT_SRC_TEXT = DEFAULT_LANG = DEFAULT_SCORE = DEFAULT_LABEL = ""
    ...
    def _from_list(
        ...
        scores, labels, speakers, src_langs, tgt_langs = [], [], [], [], []
        for s in samples:
            ...
            scores.extend([ss.get(cls.KEY_SCORE, cls.DEFAULT_SCORE) for ss in s])
