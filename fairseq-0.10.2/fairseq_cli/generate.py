...
def _main(args, output_file):
    ...
    if args.phoneme:
        scorer_p2g = scoring.build_scorer(args, tgt_dict)
        ...
        for i, sample_id in enumerate(sample["id"].tolist()):
            ...
            for j, hypo in enumerate(hypos[i][: args.nbest]):
                ...
                if not args.quiet:
                    ...
                    #                           " ".join(
                    #                             map(
                    #                             lambda x:"{:.3f}".format(x), hypo['ps'].tolist()
                    #                             ),
                    #                           )
                    #                         ),
                    #     file=output_file,
                    ...
                    if args.phoneme:
                        target_gs=p2g(args.data,target_str,args.gen_subset,sample_id)
                        print("T-p2g-{}\t{}".format(sample_id, target_gs), file=output_file)
                        hypo_gs=p2g(args.data,hypo_str,args.gen_subset,sample_id)
                        print("H-p2g-{}\t{}".format(sample_id, hypo_gs), file=output_file)
                ...
                if has_target and j == 0:
                    ...
                    if hasattr(scorer, "add_string"):
                        ...
                        ## TODO:
                        if args.phoneme:
                            scorer_p2g.add_string(target_gs, hypo_gs)
                    else:
    ...
    if has_target:
        ...
        if args.phoneme:
            print(
            "p2g:Generate {} with beam={}: {}".format(
                args.gen_subset, args.beam, scorer_p2g.result_string()
            ),
            file=output_file,
        )            
